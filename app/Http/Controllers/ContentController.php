...
class ContentController extends Controller
{
  public function index(Request $request)
  {
    ...
    $subs = Sub::with('contents', 'standard')->has('contents')->whereHas('contents', function ($content) {
        return $content->with('user')->whereHas('user', function ($user) {
            return $user->where('major_id', \Auth::user()->major_id);
        });
  ...
  public function store(StoreContentRequest $request)
  {
    $data = Sub::with('standard')->where('id', $request->get('subId'))->first();
      ...
      $path = time() . "_" . "Standard" . "_" . $data->standard->number . "_" . "Sub" . "_" . $data->number . "_" . str_replace(" ", "_", \Auth::user()->name) . "." . $request->file('content')->extension();

    if ($request->hasFile('content')) {
      $request->file('content')->store('content', 'public');
    }

      Content::create(['sub_id' => $request->get('subId'), 'user_id' => \Auth::user()->id, 'description' => $request->get('description'), 'content' => $path]);

      return Redirect::route('standards.index')->with('status', 'Menambahkan dokumen baru.');
  }
